...
char cacheDir[MAX_PATH]; //for storing the cache directory
...
//default servers
char Server1Name[MAX_PATH] = "Smolderforge";
char Server1Realm[MAX_PATH] = "connect.smolderforge.com";
char Server1Website[MAX_PATH] = "https://www.smolderforge.com";
char Server2Name[MAX_PATH] = "HellGround";
char Server2Realm[MAX_PATH] = "logon.wow-hellground.com";
char Server2Website[MAX_PATH] = "https://wow-hellground.com";
char Server3Name[MAX_PATH] = "Looking4Group";
char Server3Realm[MAX_PATH] = "logon.hellfire-tbc.com";
char Server3Website[MAX_PATH] = "https://hellfire-tbc.com/";
char Server4Name[MAX_PATH] = "ExcaliburWoW";
char Server4Realm[MAX_PATH] = "exwow-serv.exnw.com";
char Server4Website[MAX_PATH] = "http://www.excalibur-server.com/";
char Server5Name[MAX_PATH] = "Feenix (wow-one)";
char Server5Realm[MAX_PATH] = "vanillafeenix.servegame.org";
char Server5Website[MAX_PATH] = "http://www.classicwow.com/";
...
char* GetCacheDir() //cache directory is localized in tbc
{
    ...
    strcpy(cacheDir, ownPth);
    std::strcat(cacheDir,"\\Cache\\WDB\\enUS");
    if (dirExists(cacheDir))
    {
        return cacheDir;
    }
    strcpy(cacheDir, ownPth);
    std::strcat(cacheDir,"\\Cache\\WDB\\enGB");
    if (dirExists(cacheDir))
    {
        return cacheDir;
    }
    strcpy(cacheDir, ownPth);
    std::strcat(cacheDir,"\\Cache\\WDB\\esES");
    if (dirExists(cacheDir))
    {
        return cacheDir;
    }
    strcpy(cacheDir, ownPth);
    std::strcat(cacheDir,"\\Cache\\WDB\\esMX");
    if (dirExists(cacheDir))
    {
        return cacheDir;
    }
    strcpy(cacheDir, ownPth);
    std::strcat(cacheDir,"\\Cache\\WDB\\ruRU");
    if (dirExists(cacheDir))
    {
        return cacheDir;
    }
    strcpy(cacheDir, ownPth);
    std::strcat(cacheDir,"\\Cache\\WDB\\frFR");
    if (dirExists(cacheDir))
    {
        return cacheDir;
    }
    strcpy(cacheDir, ownPth);
    std::strcat(cacheDir,"\\Cache\\WDB\\deDE");
    if (dirExists(cacheDir))
    {
        return cacheDir;
    }
    strcpy(cacheDir, ownPth);
    std::strcat(cacheDir,"\\Cache\\WDB\\zhCN");
    if (dirExists(cacheDir))
    {
        return cacheDir;
    }
    strcpy(cacheDir, ownPth);
    std::strcat(cacheDir,"\\Cache\\WDB\\zhTW");
    if (dirExists(cacheDir))
    {
        return cacheDir;
    }
    strcpy(cacheDir, ownPth);
    std::strcat(cacheDir,"\\Cache\\WDB\\koKR");
    if (dirExists(cacheDir))
    {
        return cacheDir;
    }
    
    //can't find the cache folder
    MessageBox(hwnd1, "Cache directory not found!","WoW Launcher", MB_OK | MB_ICONERROR);
    strcpy(cacheDir, "Z:\\Invalid");
    return cacheDir;
}
